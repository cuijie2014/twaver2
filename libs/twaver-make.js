...
make.Utils3D = {
    ...
    createShadowImage: function (wall, floor, visibleFilter, shadowParams) {
        ...
        var scale = maxValue > 4096 ? Math.ceil(maxValue / 4096) : 1;
        // var scale = 1;
        ...
        var maxValue = Math.max(2048, Math.max(floorWidth / scale, floorHeight / scale));
        var newscale = maxValue / 2048;
        var newcanvas = make.Default.newcanvas || document.createElement('canvas');

        //地板的宽高缩放成2的幂，这样贴图就不会闪
        var fWidth = mono.Utils.nextPowerOfTwo(floorWidth / newscale);
        var fHeight = mono.Utils.nextPowerOfTwo(floorHeight / newscale);
...
var panelDef = {
    ...
    //"IBM_3850_X5_back":{label: "IBM_3850_X5_back", width: 450.00, height:180,size: 4},
    ...
    "IBM_X3850_X5": { label: "IBM_X3850_X5", width: 450.03, height: 177.81, size: 4 },
    ...
    "IBM_X3850_X5_back": { label: "IBM_X3850_X5_back", width: 450.03, height: 177.81, size: 4 },
    ...
    "SUNSEA_ODU1_back": { label: "SUNSEA_ODU1_back", width: 450.03, height: 44.50, size: 1 },
    "SUNSEA_ODU1": { label: "SUNSEA_ODU1", width: 450.03, height: 44.50, size: 1 },
    ...
    if (param.modelType == 'objModel') {
        ...
        if (param.height > 8) {
            make.Default.copy(param.id, 'twaver.idc.equipment', {
                frontImage: param.path + '/' + name + '/' + name + "_front.jpg",
                backImage: param.backData && (param.path + '/' + name + '/' + name + "_back.jpg")
            }, getModelParameters(param));
        } else {
